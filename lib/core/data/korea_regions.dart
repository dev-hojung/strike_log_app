// 한국 행정구역 정적 데이터.
//
// 클럽 생성/탐색 시 활동 지역 입력·필터에 사용.
// 시/도(17개) + 각 시/도의 시/군/구. 행정안전부 표준에 맞춘 단일 표기.
...
/// 시/도 목록 (드롭다운 1단계).
List<String> get kProvinces => kKoreaRegions.keys.toList();

/// 특정 시/도의 시/군/구 목록.
List<String> citiesOf(String? province) {
  if (province == null) return const [];
  return kKoreaRegions[province] ?? const [];
}

/// 시/도 + 시/군/구 합쳐 단일 문자열로 (백엔드 저장 포맷).
String? composeRegion(String? province, String? city) {
  if (province == null || province.isEmpty) return null;
  if (city == null || city.isEmpty) return province;
  return '$province $city';
}

/// 저장된 단일 문자열을 시/도·시/군/구로 분해.
(String?, String?) splitRegion(String? region) {
  if (region == null || region.isEmpty) return (null, null);
