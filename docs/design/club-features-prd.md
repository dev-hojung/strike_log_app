...
| # | 요구 | 비고 |
|---|---|---|
| F1-1 | 정기전 생성: 이름("6월 4주차 정모")·날짜·목표 게임수·참가자 선택 | ADMIN 전용 |
| F1-2 | 참가자 목록 관리(추가/제거) | 클럽 멤버 중 선택 |
...
POST  /groups/:id/events/:eid/participants      참가자 추가 (ADMIN)
